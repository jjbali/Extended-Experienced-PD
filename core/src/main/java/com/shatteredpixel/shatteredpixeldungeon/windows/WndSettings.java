...
public class WndSettings extends WndTabbed {
				...
				protected void onClick() {
					ShatteredPixelDungeon.scene().addToFront(new Window() {
...

						{
							...
							chkOther = new CheckBox(Messages.get(WndSettings.SeedfinderTab.this, "other")){
								@Override
								protected void onClick() {
									super.onClick();
									SPDSettings.logOther(checked());
								}
							};
							chkOther.checked(SPDSettings.logOther());
							add(chkOther);
							...
							chkOther.setRect(0, chkMisc.bottom() + GAP, width, BTN_HEIGHT);

							sep2.size(width, 1);
							sep2.y = chkMisc.bottom() + GAP;

							//chkRooms.setRect(0, sep2.y + 1 + GAP, width, BTN_HEIGHT);
							chkOther.setRect(0, sep2.y + 1 + GAP, width, BTN_HEIGHT);
							chkBlacklist.setRect(0, chkOther.bottom()+1, width, BTN_HEIGHT);
	...
	private static class DataTab extends Component{
		...
		protected void layout() {
			...
			if (hpMod != null && Dungeon.depth > 1){
				hpMod.setRect(0, pos + GAP, width, BTN_HEIGHT);
				pos = hpMod.bottom();
			}

			if (debuggable != null){
				debuggable.setRect(0, pos + GAP, width, BTN_HEIGHT);
				pos = debuggable.bottom();
			}
