...
public class Badges {
	...
	public static void validateGoldCollected() {
		...
		if (!local.contains( Badge.GOLD_COLLECTED_1 ) && Statistics.goldCollected >= 1000000L) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_1;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_2 ) && Statistics.goldCollected >= 200500000L) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_2;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_3 ) && Statistics.goldCollected >= 1700500000L) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_3;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_4 ) && Statistics.goldCollected >= 500700500000L) {
			if (badge != null) unlock(badge);
			badge = Badge.GOLD_COLLECTED_4;
			local.add( badge );
		}
		if (!local.contains( Badge.GOLD_COLLECTED_5 ) && Statistics.goldCollected >= 23500700500000L) {
			if (badge != null) unlock(badge);
	...
	public static void validatePowerReached() {
		Badge badge = null;
		if (!local.contains( Badge.SECOND_ARENA_MASTER) && Objects.equals(Dungeon.level, new SecondArenaLevel())) {
			badge = Badge.SECOND_ARENA_MASTER;
			local.add( badge );
		}
		if (!local.contains( Badge.THIRD_ARENA_MASTER) && Objects.equals(Dungeon.level, new ThirdArenaLevel())) {
			if (badge != null) unlock(badge);
			badge = Badge.THIRD_ARENA_MASTER;
			local.add( badge );
		}
		if (!local.contains( Badge.FOURTH_ARENA_MASTER) && Objects.equals(Dungeon.level, new FourthArenaLevel())) {
			if (badge != null) unlock(badge);
			badge = Badge.FOURTH_ARENA_MASTER;
			local.add( badge );
		}
		displayBadge( badge );
	}
	...
	public static void validateStrengthAttained() {
		...
		if (!local.contains( Badge.STRENGTH_ATTAINED_1 ) && Dungeon.hero.STR >= 100) {
	...
	public static void validateItemsCrafted() {
		...
		if (!local.contains( Badge.ITEMS_CRAFTED_1 ) && Statistics.itemsCrafted >= 100) {
			badge = Badge.ITEMS_CRAFTED_1;
			local.add( badge );
		}
		if (!local.contains( Badge.ITEMS_CRAFTED_2 ) && Statistics.itemsCrafted >= 200) {
			if (badge != null) unlock(badge);
	...
	public static void validatePiranhasKilled() {
		...
		if (!local.contains( Badge.PIRANHAS ) && Statistics.piranhasKilled >= 50) {
	...
	public static void validateItemLevelAquired( Item item ) {
		...
		Badge badge = null;
		if (!local.contains( Badge.ITEM_LEVEL_1 ) && item.level() >= 250) {
			badge = Badge.ITEM_LEVEL_1;
			local.add( badge );
		}
		if (!local.contains( Badge.ITEM_LEVEL_2 ) && item.level() >= 500) {
			if (badge != null) unlock(badge);
			badge = Badge.ITEM_LEVEL_2;
			local.add( badge );
		}
		if (!local.contains( Badge.ITEM_LEVEL_3 ) && item.level() >= 750) {
			if (badge != null) unlock(badge);
			badge = Badge.ITEM_LEVEL_3;
			local.add( badge );
		}
		if (!local.contains( Badge.ITEM_LEVEL_4 ) && item.level() >= 1000) {
			if (badge != null) unlock(badge);
	...
	public static void validateGamesPlayed() {
		Badge badge = null;
		if (Rankings.INSTANCE.totalNumber >= 50 || Rankings.INSTANCE.wonNumber >= 10) {
			badge = Badge.GAMES_PLAYED_1;
		}
		if (Rankings.INSTANCE.totalNumber >= 100 || Rankings.INSTANCE.wonNumber >= 20) {
			unlock(badge);
			badge = Badge.GAMES_PLAYED_2;
		}
		if (Rankings.INSTANCE.totalNumber >= 150 || Rankings.INSTANCE.wonNumber >= 30) {
			unlock(badge);
			badge = Badge.GAMES_PLAYED_3;
		}
		if (Rankings.INSTANCE.totalNumber >= 200 || Rankings.INSTANCE.wonNumber >= 40) {
			unlock(badge);
			badge = Badge.GAMES_PLAYED_4;
		}
		if (Rankings.INSTANCE.totalNumber >= 1000 || Rankings.INSTANCE.wonNumber >= 60) {
			unlock(badge);
