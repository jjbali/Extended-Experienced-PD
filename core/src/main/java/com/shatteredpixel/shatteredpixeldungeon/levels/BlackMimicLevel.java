...
public class BlackMimicLevel extends Level {
	...
	public String tileDesc( int tile ) {
		switch (tile) {
			case Terrain.WATER:
				return super.tileDesc( tile ) + "\n\n" + Messages.get(BlackMimicLevel.class, "water_desc");
	...
	private void buildEntrance(){
		...
		transitions.add(new LevelTransition(this, entrance, LevelTransition.Type.BRANCH_ENTRANCE, 100, Dungeon.BRANCH_NORMAL, LevelTransition.Type.REGULAR_EXIT));
	}
	...
	public static class ArenaVisuals extends CustomTilemap {
		...
		public String name(int tileX, int tileY) {
			...
			if (Dungeon.level.map[i] == Terrain.INACTIVE_TRAP){
				return Messages.get(BlackMimicLevel.class, "wires_name");
			} else if (gate.inside(Dungeon.level.cellToPoint(i))){
				return Messages.get(BlackMimicLevel.class, "gate_name");
			}
		...
		public String desc(int tileX, int tileY) {
			int i = tileX + tileW*(tileY + this.tileY);
			if (Dungeon.level.map[i] == Terrain.INACTIVE_TRAP){
				return Messages.get(BlackMimicLevel.class, "wires_desc");
			} else if (gate.inside(Dungeon.level.cellToPoint(i))){
				if (Dungeon.level.solid[i]){
					return Messages.get(BlackMimicLevel.class, "gate_desc");
				} else {
					return Messages.get(BlackMimicLevel.class, "gate_desc_broken");
				}
			}
	...
	public static class PylonEnergy extends Blob {
		...
		public String tileDesc() {
			return Messages.get(BlackMimicLevel.class, "energy_desc");
		}
