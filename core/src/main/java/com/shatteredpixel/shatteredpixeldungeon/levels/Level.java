...
import com.shatteredpixel.shatteredpixeldungeon.actors.mobs.DimensionalBestiary;
...
public abstract class Level implements Bundlable {
			...
			if (Dungeon.depth > 1) {
				...
				switch (Random.Int( 25 )) {
					...
					case 6: case 15:
						...
						break;
					case 7:
						feeling = Feeling.INFINITY;
						break;
					case 8:
						feeling = Feeling.CHAMPION;
						break;
				}
			}
		}
	...
	public float respawnCooldown(){
		...
		if (Statistics.amuletObtained){
			if (Dungeon.depth == 1){
				...
				return (Dungeon.level.mobCount()) * (respawn_timer / 25f);
			} else {
				//respawn time is 5/5/10/15/20/25/25, etc.
				return Math.round(GameMath.gate( respawn_timer/10f, Dungeon.level.mobCount() * (TIME_TO_RESPAWN / 10f), TIME_TO_RESPAWN / 2f));
			}
		} else if (Dungeon.level.feeling == Feeling.DARK){
			return 2* respawn_timer /3f;
		} else {
	...
	public Item findPrizeItem(Class<?extends Item> match){
		...
		if (match == null){
			...
			for (Item i : itemsToSpawn){
				if (i instanceof FragmentCatalyst){
	...
	public boolean setCellToGround( boolean includeTraps, int cell ){
		Point p = cellToPoint(cell);
		...
		if (terr == Terrain.CHASM){
			Level.set(cell, Random.Int(2) == 0 ? Terrain.EMPTY_SP : Terrain.EMPTY);
			GameScene.updateMap(cell);
			return true;
		} else if (includeTraps && (terr == Terrain.SECRET_TRAP ||
				terr == Terrain.TRAP || terr == Terrain.INACTIVE_TRAP)){
	...
	public void updateFieldOfView( Char c, boolean[] fieldOfView ) {
		...
		if (sighted) {
			...
			ShadowCaster.castShadow( cx, cy, Dungeon.level.width(), fieldOfView, blocking, viewDist );
		} else {
