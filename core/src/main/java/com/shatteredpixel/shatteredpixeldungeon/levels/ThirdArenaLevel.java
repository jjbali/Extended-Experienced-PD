...
import com.shatteredpixel.shatteredpixeldungeon.levels.rooms.special.ThirdArenaShopLevel;
...
public class ThirdArenaLevel extends Level {
	...
	private ThirdArenaShopLevel shop;
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		shop = (ThirdArenaShopLevel) bundle.get(SHOP);
	}
	...
	protected boolean build() {
...

		{
			...
			itemsToSpawn.add(Generator.random(Generator.Category.WAND).upgrade(Dungeon.IntRange(1, 1000)));
	...
	public static class ThirdArenaCounter extends CounterBuff {}
	
	@Override
	public Respawner respawner() {
        return new ThirdArenaRespawner();
    }

    public static class ThirdArenaBuff extends Buff {
	...
	public static class ThirdArenaRespawner extends Respawner {
		...
		protected boolean act() {
			...
			ThirdArenaCounter counter = Dungeon.hero.buff(ThirdArenaCounter.class);
			float timerBasis;
			timerBasis = 30f;
			...
			if (count < 50) {
				...
				if (Dungeon.hero.isAlive() && mob.pos != -1 && Dungeon.level.distance(Dungeon.hero.pos, mob.pos) >= 4) {
					...
					Buff.affect(mob, ThirdArenaBuff.class);
					if (counter != null){
