...
import com.shatteredpixel.shatteredpixeldungeon.levels.rooms.special.FourthArenaShopLevel;
...
public class FourthArenaLevel extends Level {
	...
	private FourthArenaShopLevel shop;
	...
	public void restoreFromBundle( Bundle bundle ) {
		...
		shop = (FourthArenaShopLevel) bundle.get(SHOP);
	}
	...
	protected boolean build() {
...

		{
			...
			itemsToSpawn.add(new RandomItemTicket().quantity(1000));
			itemsToSpawn.add(Generator.random(Generator.Category.TREASUREBAG));
			itemsToSpawn.add(Generator.random(Generator.Category.WEP_T5));
			itemsToSpawn.add(new Cheese().quantity(1500));
			itemsToSpawn.add(new ChaosFishingRod().upgrade(150));
			itemsToSpawn.add(new ArcaneBomb().quantity(1500));
			itemsToSpawn.add(new PlateArmor().upgrade(1500));
	...
	public static class FourthArenaCounter extends CounterBuff {}
	
	@Override
	public Respawner respawner() {
        return new FourthArenaRespawner();
    }

    public static class FourthArenaBuff extends Buff {
	...
	public String tileDesc( int tile ) {
		switch (tile) {
			case Terrain.ENTRANCE:
				return Messages.get(SewerLevel.class, "entrance_desc");
			case Terrain.EXIT:
				return Messages.get(SewerLevel.class, "exit_desc");
			case Terrain.HIGH_GRASS:
	...
	public static class FourthArenaRespawner extends Respawner {
		...
		protected boolean act() {
			...
			FourthArenaCounter counter = Dungeon.hero.buff(FourthArenaCounter.class);
			float timerBasis;
			timerBasis = 50f;
			...
			if (count < 150) {
				...
				if (Dungeon.hero.isAlive() && mob.pos != -1 && Dungeon.level.distance(Dungeon.hero.pos, mob.pos) >= 4) {
					...
					Buff.affect(mob, FourthArenaBuff.class);
					if (counter != null){
						...
						if (power >= 50){
							for (int i = 0; i < Math.min(5, power); i++){
