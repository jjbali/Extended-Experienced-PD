...
import com.shatteredpixel.shatteredpixeldungeon.levels.rooms.special.ItemShopRoom;
...
public abstract class RegularLevel extends Level {
	...
	protected ArrayList<Room> initRooms() {
		...
		if (Dungeon.shopOnLevel3() && SubtractionItem.streak_b >= 50)
	...
	protected void createItems() {
		...
		// drops 11/13/16 items 60%/30%/10% of the time
		int nItems = 15 + Random.chances(new float[]{6, 3, 1});
		...
		for (Item item : itemsToSpawn) {
			int cell = randomDropCell();
			if (item instanceof FragmentCatalyst){
		...
		if (Document.ADVENTURERS_GUIDE.allPagesFound()){
			...
			int region = 1 + Math.round((Dungeon.depth/5));
			...
			switch( region ){
				...
				case 1: regionDoc = Document.ONE; break;
				case 2: regionDoc = Document.TWO; break;
			}
