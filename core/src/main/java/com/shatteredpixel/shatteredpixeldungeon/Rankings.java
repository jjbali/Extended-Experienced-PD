...
public enum Rankings {
	...
	private long score( boolean win ) {
		return (Statistics.goldCollected + Dungeon.hero.lvl * (win ? 101 : Dungeon.depth ) * 100) * (win ? 2 : 1);
	}
	...
	public long calculateScore(){
		...
		if (Dungeon.initialVersion > 453){
			...
			Statistics.streakMultiplier = 0f;
		...
		//win multiplier is a simple 2x if run was a win, challenge multi is the same as 1.3.0
			...
			Statistics.treasureScore = Math.min(Statistics.goldCollected, 150_000);
