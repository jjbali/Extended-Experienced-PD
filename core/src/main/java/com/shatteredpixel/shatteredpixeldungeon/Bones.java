...
public class Bones {
	...
	public static void leave() {
		...
		// but are capped at 20 floors above the lowest depth reached (even when ascending)
		depth = Math.max(Dungeon.depth, Statistics.deepestFloor-20);
	...
	public static Item get() {
		...
		if (depth == -1) {
			...
			if (lootAtCurLevel()) {
				...
				//challenged or seeded runs will always find 1000 gold
	...
	private static boolean lootAtCurLevel(){
		if (branch == Dungeon.branch) {
			if (branch == 0) {
				...
				return depth/20 == Dungeon.depth/20;
			}
		}
		return false;
	}
