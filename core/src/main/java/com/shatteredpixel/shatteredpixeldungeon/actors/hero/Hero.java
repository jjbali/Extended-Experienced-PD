...
import com.shatteredpixel.shatteredpixeldungeon.items.armor.LiminalArmor;
...
public class Hero extends Char {
	...
	public void updateHT( boolean boostHP ){
		...
		float extender = HealthModule.multi;
		if (extender >= 1) {
			HT += (50 * extender);
		}
		...
		if (SPDSettings.healthMod() > 1) {
			HP = HT *= SPDSettings.healthMod();
		}
	...
	public int STR() {
		...
		strBonus += Math.ceil(MultiplicationItem.totalAnswers_c/10);
	...
	public String name(){
		...
		return customHeroName + ", " + className();
	}
	@Override
	...
	public int attackSkill( Char target ) {
		...
		float accuracy = 3;
		accuracy += (float) DivisionItem.totalAnswers_d;
	...
	public long damageRoll() {
		...
		if (!RingOfForce.fightingUnarmed(this)) {
			...
			if (RingOfForce.unarmedGetsWeaponAugment(this)){
				dmg = ((Weapon)belongings.attackingWeapon()).augment.damageFactor((int) dmg);
			}
		}
					...
					public void call() {
						...
						if (Dungeon.level.map[action.dst] == Terrain.WALL_DECO){
							...
							if (gold.doPickUp( hero )) {
								GLog.i( Messages.capitalize(Messages.get(hero, "you_now_have", gold.name())) );
							} else {
				...
				() -> {
					for (int i = 0; i < PathFinder.distance.length; i++) {
						if (PathFinder.distance[i] < Integer.MAX_VALUE) {
							...
							if (ch != null && ch.alignment != alignment && ch != hero) {
	...
	public boolean handle( int cell ) {
		...
		//TODO perhaps only trigger this if hero is already adjacent? reducing mistaps
			...
			curAction = new HeroAction.Unlock( cell );
				...
				// TODO another fixme issue heheh
				&& ((Dungeon.depth < 999 || (Dungeon.branch == Dungeon.BRANCH_BLACK && hero.belongings.getItem(Amulet.class) != null))
	...
	public void earnExp(long exp, Class source ) {
		...
		if (levelUp) {
			...
			if (sprite != null) {
				...
				hero.sprite.showStatusWithIcon(CharSprite.POSITIVE, "1", FloatingText.LEVEL_UP);
	...
	public static void getExp(long value){
		hero.earnExp(value, null);
	}
