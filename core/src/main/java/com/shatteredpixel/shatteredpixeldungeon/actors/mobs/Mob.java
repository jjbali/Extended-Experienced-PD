...
public abstract class Mob extends Char {
		...
		if (buff(SoulMark.class) != null) {
			...
			if (restoration > 0) {
				if (hero.subClass == HeroSubClass.WARLOCK)
					Buff.affect(hero, Hunger.class).affectHunger(restoration*1.5f);
	...
	public void die( Object cause ) {
		...
		if (!(this instanceof Wraith)
				&& soulMarked
				&& Random.Float() < (hero.subClass == HeroSubClass.WARLOCK ? 0.5f : 0)){
	...
	public float lootChance(){
		...
		float dropBonus = RingOfLuck.dropChanceMultiplier( Dungeon.hero );
	...
	public void rollToDropLoot(){
		...
		if (hero.grinding && hero.heroClass == HeroClass.ROGUE){
			...
			if (Random.Float() < 0.2f) Dungeon.level.drop(new Bomb(), pos).sprite.drop();
		}
		if (hero.grinding && hero.heroClass == HeroClass.HUNTRESS){
			int quantity = Dungeon.IntRange(2, 4);
...
//		//ring of wealth logic
		...
		if (Ring.getBonus(hero, RingOfLuck.Wealth.class) > 0) {
			int rolls = 5 * Ring.getBonus(hero, RingOfLuck.Wealth.class);
			if (properties.contains(Property.BOSS)) rolls = 20 * Ring.getBonus(hero, RingOfLuck.Wealth.class);
			else if (properties.contains(Property.MINIBOSS)) rolls = 10 * Ring.getBonus(hero, RingOfLuck.Wealth.class);
		...
		//soul eater talent
		if (buff(SoulMark.class) != null &&
				Random.Int(10) < (hero.subClass == HeroSubClass.WARLOCK ? 4 : 0)){
