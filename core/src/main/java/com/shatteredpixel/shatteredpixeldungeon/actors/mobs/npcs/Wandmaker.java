...
public class Wandmaker extends NPC {
	...
	public boolean interact(Char c) {
		...
		if (Dungeon.hero.belongings.getItem(Cheese.class) != null){
			...
			return true;
		} else if (Dungeon.hero.belongings.getItem(TicketToFourthArena.class) != null){
			Game.runOnRenderThread(new Callback() {
				@Override
				public void call() {
					GameScene.show(new WndQuest(Wandmaker.this, Messages.get(Wandmaker.class, "ticket")){
						@Override
						public void hide() {
							super.hide();
							...
							Dungeon.hero.belongings.getItem(TicketToFourthArena.class).detach(Dungeon.hero.belongings.backpack);
							Item rod = new TheTrueFishingRod();
							if (rod.doPickUp( Dungeon.hero )) {
								GLog.i( Messages.get(Dungeon.hero, "you_now_have", rod.name()) );
							} else {
								Dungeon.level.drop( rod, pos ).sprite.drop();
							}
							yell( Messages.get(this, "farewell", Dungeon.hero.name()) );
							Wandmaker.this.destroy();
	...
	public static class Quest {
		...
		public static void complete() {
			...
			Statistics.questScores[1] = 6000;
			Questionnaire.points_gathered += 5000;
		}
	}
}
