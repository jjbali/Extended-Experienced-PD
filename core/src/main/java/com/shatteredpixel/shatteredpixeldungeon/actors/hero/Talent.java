...
//		}
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ){
		...
		switch (cls){
			...
			case RAT_KING:
				Collections.addAll(tierTalents, HEARTY_MEAL, VETERANS_INTUITION, TEST_SUBJECT, IRON_WILL,
						EMPOWERING_MEAL, SCHOLARS_INTUITION, TESTED_HYPOTHESIS,
						CACHED_RATIONS, THIEFS_INTUITION, SUCKER_PUNCH, PROTECTIVE_SHADOWS,
						NATURES_BOUNTY, SURVIVALISTS_INTUITION, FOLLOWUP_STRIKE,
		...
		switch (cls){
			...
			case DUELIST:
				...
				break;
			case RAT_KING:
				Collections.addAll(tierTalents, IRON_STOMACH, RESTORED_WILLPOWER, LETHAL_MOMENTUM, ENERGIZING_MEAL,
						WAND_PRESERVATION, MYSTICAL_MEAL, ROGUES_FORESIGHT,
						INVIGORATING_MEAL, RESTORED_NATURE, DURABLE_PROJECTILES,
						FOCUSED_MEAL, RESTORED_AGILITY, LETHAL_HASTE, SWIFT_EQUIP);
				break;
		}
		for (Talent talent : tierTalents){
			if (replacements.containsKey(talent)){
		...
		switch (cls){
			...
			case DUELIST:
				...
				break;
			case RAT_KING:
				Collections.addAll(tierTalents, HOLD_FAST, DESPERATE_POWER,
						ALLY_WARP, ENHANCED_RINGS,
						POINT_BLANK, SEER_SHOT,
						PRECISE_ASSAULT, DEADLY_FOLLOWUP);
				break;
		}
		for (Talent talent : tierTalents){
			if (replacements.containsKey(talent)){
	...
	public static void initSubclassTalents( HeroSubClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents ){
		...
		switch (cls){
			...
			case KING:
				Collections.addAll(tierTalents, ENDLESS_RAGE, DEATHLESS_FURY, ENRAGED_CATALYST,
						CLEAVE, LETHAL_DEFENSE, ENHANCED_COMBO,
						EMPOWERED_STRIKE, MYSTICAL_CHARGE, EXCESS_CHARGE,
						SOUL_EATER, SOUL_SIPHON,
						ENHANCED_LETHALITY, ASSASSINS_REACH, BOUNTY_HUNTER,
						EVASIVE_ARMOR, PROJECTILE_MOMENTUM, SPEEDY_STEALTH,
						FARSIGHT, SHARED_ENCHANTMENT, SHARED_UPGRADES,
						DURABLE_TIPS, BARKSKIN, SHIELDING_DEW,
						SECONDARY_CHARGE, TWIN_UPGRADES,
						UNENCUMBERED_SPIRIT, MONASTIC_VIGOR, COMBINED_ENERGY);
				break;
		}
		for (Talent talent : tierTalents){
			talents.get(2).put(talent, 0);
