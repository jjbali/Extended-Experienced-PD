...
public enum Talent {
	...
	public static int[] tierLevelThresholds = new int[]{0, 2, 12, 32, 60, 85};
	...
	public static void initClassTalents( HeroClass cls, ArrayList<LinkedHashMap<Talent, Integer>> talents, LinkedHashMap<Talent, Talent> replacements ){
		...
		switch (cls){
			...
			case DUELIST:
				...
				break;
			case RAT_KING:
				Collections.addAll(tierTalents,VETERANS_INTUITION, TEST_SUBJECT, EMPOWERING_MEAL, SCHOLARS_INTUITION, QUESTIONNAIRE_SUPERVISOR);
				break;
		}
		...
		switch (cls){
			...
			case DUELIST:
				...
				break;
			case RAT_KING:
				Collections.addAll(tierTalents, LETHAL_HASTE, SWIFT_EQUIP, SCROLL_COLLECTOR, POTION_COLLECTOR, TREASUREBAG_COLLECTOR);
				break;
		}
		for (Talent talent : tierTalents){
			if (replacements.containsKey(talent)){
