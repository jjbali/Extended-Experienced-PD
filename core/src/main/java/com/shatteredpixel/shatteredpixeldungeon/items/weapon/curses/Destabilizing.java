...
public class Destabilizing extends Weapon.Enchantment {
	...
	public long proc( Weapon weapon, Char attacker, Char defender, long damage ) {
		...
		long targets = Math.max( weapon.buffedLvl(), 0 ) / 2 + 2;
		Buff.affect( defender, Destabilized.class ).targets((int) targets).addStacks( 1 );
		Buff.affect( attacker, Destabilized.class ).targets((int) targets).addStacks( 1 );
