...
public class ModuloItem extends Questionnaire {
    {
        image = ItemSpriteSheet.MODULO;
        unique = true;
        identify();
    ...
    private int CODE = Random.Int(Integer.MAX_VALUE) + 1;
    private int CODE2 = Random.Int(10) + 1;
    private String ANSWER = String.valueOf(CODE % CODE2);
    public static int totalAnswers_n = 0;
    public static int streak_n = 0;
    public int t = totalAnswers_n;
    public static int s = streak_n;
    ...
    public void execute( Hero hero, String action ) {
        ...
        if (action.equals( AC_ANSWER ) && hero.buff(CodeCooldown15.class) == null ) {
            askCode();
        } else if (action.equals( AC_ANSWER ) && hero.buff(CodeCooldown15.class) != null) {
    ...
    private void askCode() {
        GameScene.show(new WndTextInput( "Input Answer","Find The Answer: " + CODE + " % " + CODE2, "", 25, false, "Done", "Cancel" ) {
            @Override
            public void onSelect( boolean positive, String text ) {
                if (text.equals(ANSWER)) {
                    ...
                    if (hero.pointsInTalent(Talent.QUESTIONNAIRE_SUPERVISOR_II) >= 3 && Random.Int(10) == 0){
                        updateQuickslot();
                        Dungeon.level.drop(new TieredCard().upgrade(Math.round(4 + totalAnswers_n/4)), curUser.pos).sprite.drop();
                    }
                    ...
                    totalAnswers_n += 1;
                    streak_n += 1 + YellowFragment.questionnairesStreakAdd();
                    ...
                    if (Random.Float() >= 0.95f) {
                        // 5% of getting an exp
                        updateQuickslot();
                        hero.earnExp(hero.maxExp(), ModuloItem.class);
    ...
    public ItemSprite.Glowing glowing() {
        return new ItemSprite.Glowing( 0x8c2a62, 0.3f );
    }
    ...
    @Override
    public String name() {
        return "Modulo Questionnaire";
    }

    @Override
    public String desc() {
        return "This item represents your intelligence, it may also give you some rewards.\n\nFind The Answer: " + CODE + " % " + CODE2 + "\nAnswered Correctly: " + totalAnswers_n + "\nStreak: " + streak_n
    ...
    public void storeInBundle( Bundle bundle ) {
        ...
        bundle.put( STREAKS, streak_n );
        bundle.put( TOTAL_ANSWERS, totalAnswers_n );
    }

    @Override
    public void restoreFromBundle( Bundle bundle ) {
        super.restoreFromBundle( bundle );
        streak_n = bundle.getInt(STREAKS);
        totalAnswers_n = bundle.getInt(TOTAL_ANSWERS);
    }

    public static class CodeCooldown15 extends Buff {
