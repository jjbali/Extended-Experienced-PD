...
public class Temporal extends Weapon.Enchantment {
	...
	public long proc( Weapon weapon, Char attacker, Char defender, long damage ) {
		...
		dmg = AttackAmplificationBuff.damageFactor((int) dmg, attacker.buffs() );
		Buff.append( defender, Wound.class, Wound.DURATION ).set((int) dmg);
		defender.sprite.showStatus( DAMAGE_COLOR, Integer.toString((int) dmg) );
		return 0;
	}
