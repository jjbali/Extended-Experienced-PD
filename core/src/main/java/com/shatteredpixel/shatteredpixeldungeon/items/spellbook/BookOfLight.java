...
public class BookOfLight extends SpellBook {
    ...
    public void readEffect() {
        Buff.affect(Dungeon.hero, Light.class, 50*(1 + 0.5f  ));
        GameScene.flash(0xFFFFFF);
        Sample.INSTANCE.play(Assets.Sounds.BLAST);
        for (Mob m : Dungeon.level.mobs) {
            if (!(m instanceof NPC) && Dungeon.level.heroFOV[m.pos]) {
                Buff.affect(m, Blindness.class, (2+(int)Math.floor(Dungeon.hero.lvl/6f)*(1+0.5f )));
            }
        }
    }
    ...
    public String info() {
        ...
        if (Dungeon.hero.buff(SpellBookCoolDown.class) == null) {
            info += "\n\n" + Messages.get(this, "time",
                    new DecimalFormat("#").format(50*(1 + 0.5f  )),
                    new DecimalFormat("#").format((2+(int)Math.floor(Dungeon.hero.lvl/6f)*(1+0.5f ))));
        }
        return info;
    }
