...
public class PotionOfDebug extends Potion {
    ...
    @Override
    public String name() {
        return "Potion of Hero's Destiny";
    }
    ...
    private void askAmount( Item item, Consumer<Item> next ) {
        ...
        GameScene.show( new WndTextInput( "Input amount","", "", 8, false, "Done", "Cancel" ) {
            @Override
    ...
    private void askEnchant( Item item, Consumer<Item> next ) {
        if ( !(item instanceof Weapon) || item instanceof MissileWeapon) {
            next.accept( item );
            return;
        }
    ...
    public static class WndBetterOptions extends Window {
        ...
        private static final int FONT_SIZE_SM = 4;
    ...
    public static class DebugCooldown extends Buff {
