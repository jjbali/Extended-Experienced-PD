...
public class AboutScene extends PixelScene {
	...
	public void create() {
		...
		CreditsBlock tweaks = new CreditsBlock(true, TWEAKS_COLOR,
				...
				"Developed by: _Jjbali_\nAn extended version for greedy people\n\nBased on the source of ExpPD by _Trashboxbobylev_",
				null,
				null);
		if (landscape()){
			tweaks.setRect((w - fullWidth)/2f, 10, 120, 0);
		} else {
		...
		CreditsBlock shpx = new CreditsBlock(true, Window.SHPX_COLOR,
				...
				shpxLink);
		if (landscape()) {
			shpx.setRect(tweaks.left(), tweaks.bottom() + 8, colWidth, 0);
		} else {
			shpx.setRect(tweaks.left(), tweaks.bottom() + 8, colWidth, 0);
		}
		content.add(shpx);
		...
		transifex.setRect((Camera.main.width - colWidth)/2f, bob.bottom() + 12, colWidth, 0);
