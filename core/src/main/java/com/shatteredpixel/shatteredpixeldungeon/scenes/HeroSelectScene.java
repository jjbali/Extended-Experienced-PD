		...
		protected void createChildren() {
			...
			if (DeviceCompat.isDebug() || Badges.isUnlocked(Badges.Badge.VICTORY) || Badges.isUnlocked(Badges.Badge.BOSS_SLAIN_1)){
				...
				StyledButton btnRenameHero = new StyledButton(Chrome.Type.BLANK, "Rename", 6){
					@Override
					protected void onClick() {
						ShatteredPixelDungeon.scene().addToFront(new WndTextInput("Rename Hero", "", Hero.customHeroName, 50, false, "Done", "Cancel") {
							@Override
							public void onSelect( boolean positive, String text ) {
								if (text != null && positive) {
									Hero.customHeroName = text;
									icon(Icons.get(Hero.customHeroName.isEmpty() ? Icons.RENAME_OFF : Icons.RENAME_ON));
								}
							}
