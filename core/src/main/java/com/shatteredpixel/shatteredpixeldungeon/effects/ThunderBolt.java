...
public class ThunderBolt extends Group {
	...
	public ThunderBolt(PointF from, int to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public ThunderBolt(int from, PointF to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public ThunderBolt(PointF from, PointF to, Callback callback){
		this(Arrays.asList(new Arc(from, to)), callback);
	}

	public ThunderBolt(List<Arc> arcs, Callback callback ) {
	...
	public static class Arc extends Group {
		...
		public Arc(PointF from, PointF to){
			...
			arc1 = new Image(Effects.get(Effects.Type.LIGHT_RAY));
			arc1.x = start.x - arc1.origin.x;
			arc1.y = start.y - arc1.origin.y;
			arc1.origin.set( 0, arc1.height()/2 );
			add( arc1 );

			arc2 = new Image(Effects.get(Effects.Type.LIGHT_RAY));
